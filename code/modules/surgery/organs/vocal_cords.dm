		...
		for(var/mob/living/carbon/C in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					...
					E.cooldown += 10
					addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(to_chat), C, span_notice("Drowsiness suddenly overwhelms you as you fall asleep!</b>")), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					L.lay_down()
					E.cooldown += 10
					addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(to_chat), L, "[E.lewd ? span_love("You eagerly lie down!") : span_notice("You suddenly lie down!")]"), 5)
		...
		for(var/V in listeners)
			...
			if(E.phase > 1)
				if(user.ckey == E.enthrallID && user.real_name == E.master.real_name)
					E.master = user
					addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(to_chat), H, span_nicegreen("[(E.lewd?"You hear the words of your [E.enthrallGender] again!! They're back!!":"You recognise the voice of [E.master].")]</b>")), 5)

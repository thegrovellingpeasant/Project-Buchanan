...
/datum/surgery_step/fix_brain/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(target.getorganslot(ORGAN_SLOT_BRAIN))
		...
		user.visible_message(
			message = span_warning("[user] suddenly notices that the brain [user.p_they()] [user.p_were()] working on is not there anymore."),
			self_message = span_warning("You suddenly notice that the brain you were working on is not there anymore."),
		)
	return FALSE

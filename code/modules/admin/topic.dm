...
/datum/admins/Topic(href, href_list)
	...
	if(href_list["editrightsbrowsermanage"])
		...
		message_admins(span_adminnotice("[key_name_admin(usr)] edited the plane's timeleft to [timer] seconds."))
		href_list["secrets"] = "check_antagonist"
		...
		if(jobban_isbanned(M, "appearance"))
			switch(alert("Remove appearance ban?","Please Confirm","Yes","No"))
				if("Yes")
					...
					to_chat(M, span_boldannounce("<BIG>[usr.client.key] has removed your appearance ban.</BIG>"))
			...
			if("Yes")
				...
				to_chat(M, span_boldannounce("<BIG>You have been appearance banned by [usr.client.key].</BIG>"))
		...
		if(notbannedlist.len) //at least 1 unbanned job exists in joblist so we have stuff to ban.
			...
			switch(alert("Temporary Ban for [M.key]?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					to_chat(M, span_boldannounce("<BIG>You have been [(msg == ("ooc" || "appearance")) ? "banned" : "jobbanned"] by [usr.client.key] from: [msg].</BIG>"))
					to_chat(M, span_boldannounce("The reason is: [reason]"))

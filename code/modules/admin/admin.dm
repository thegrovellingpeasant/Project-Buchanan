...
/datum/admins/proc/toggleaooc()
	...
	set name="Toggle Faction OOC"
	toggle_aooc()
	log_admin("[key_name(usr)] toggled Faction OOC.")
	message_admins("[key_name_admin(usr)] toggled Faction OOC.")
	SSblackbox.record_feedback("nested tally", "admin_toggle", 1, list("Toggle Faction OOC", "[GLOB.aooc_allowed ? "Enabled" : "Disabled"]")) //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!

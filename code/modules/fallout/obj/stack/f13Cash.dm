...
/obj/item/stack/f13Cash/caps/suicide_act(mob/user)
	if(src.amount <= 100)
		user.visible_message(span_suicide("[user] cannot live with being poor anymore! It looks like [user.p_theyre()] trying to commit suicide!"))
		return BRUTELOSS
	user.visible_message(span_suicide("Money couldn't buy [user] happiness! It looks like [user.p_theyre()] trying to commit suicide!"))

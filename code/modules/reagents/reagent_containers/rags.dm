...
/obj/item/reagent_containers/rag/afterattack(atom/A, mob/user,proximity)
	...
	if(iscarbon(A) && A.reagents && reagents.total_volume)
		...
		if(user.a_intent == INTENT_HARM && !C.is_mouth_covered())
			C.visible_message(span_danger("[user] is trying to smother \the [C] with \the [src]!"), span_userdanger("[user] is trying to smother you with \the [src]!"), span_italics("You hear some struggling and muffled cries of surprise."))
			if(do_after(user, 20, target = C))
				reagents.reaction(C, INGEST)
				reagents.trans_to(C, 5, log = TRUE)
				C.visible_message(span_danger("[user] has smothered \the [C] with \the [src]!"), span_userdanger("[user] has smothered you with \the [src]!"), span_italics("You hear some struggling and a heavy breath taken."))

...
/obj/item/light/dim // this is half assedly entered in because i only need it in a few places and don't want to bother with making them buildable right now
	name = "light tube"
	desc = "A replacement light tube."
	icon_state = "ltube"
	base_state = "ltube"
...
/obj/machinery/light/proc/flicker(amount = rand(10, 20), spark = TRUE, sounds = TRUE, loud = TRUE)
	...
	if(on && status == LIGHT_OK)
		if(loud)
			visible_message(span_warning("[src] begins flickering!"),span_italics("You hear an electrical sparking."))

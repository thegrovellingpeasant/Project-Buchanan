...
/obj/item/clothing/head/throw_impact(atom/hit_atom, datum/thrownthing/thrownthing)
	...
	if(iscarbon(hit_atom))
		...
		if(istype(H.head, /obj/item))
			...
			H.visible_message(
				message = span_warning("[src] bounces off [H]'s [WH.name]!"),
	...
	if(iscyborg(hit_atom))
		...
		if(!is_type_in_typecache(src, R.equippable_hats) || R.hat_offset == INFINITY)
			R.visible_message(
				message = span_warning("[src] bounces off [R]!"),
				self_message = span_warning("[src] bounces off you, falling to the floor."),
			)
			return
		else
			R.visible_message(
				message = span_notice("[src] lands neatly on top of [R]."),
				self_message = span_warning("[src] lands perfectly on top of you."),
			)

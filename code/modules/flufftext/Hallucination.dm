...
/datum/hallucination/stationmessage/New(mob/living/carbon/C, forced = TRUE, message)
	...
	switch(message)
		if("ufo")
			...
			to_chat(target, "<br><br>[span_alert("Is it a vertibird? Is it a plane? Nobody knows!")]<br><br>")
		...
		if("revelation") //AI is doomsdaying!
			...
			to_chat(target, "<br><br>[span_alert("You feel different... Changed even! You can finally think clearly, it all makes sense now!")]<br><br>")
		...
		if("nuke") //Meteors inbound!
			...
			to_chat(target, "<br><br>[span_alert("WHAT?! OH NO! THIS IS IT! THE END IS COMING!")]<br><br>")
			SEND_SOUND(target, get_announcer_sound("meteors"))
		if("supermatter")
...
/obj/effect/hallucination/danger/lava/proc/on_entered(datum/source, atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	SIGNAL_HANDLER
	if(arrived == target)
...
/obj/effect/hallucination/danger/chasm/proc/on_entered(datum/source, atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	SIGNAL_HANDLER
	if(arrived == target)
...
/obj/effect/hallucination/danger/anomaly/Initialize(mapload, _target)
	. = ..()
	START_PROCESSING(SSobj, src)
	var/static/list/loc_connections = list(
		COMSIG_ATOM_ENTERED = PROC_REF(on_entered),
...
/obj/effect/hallucination/danger/anomaly/proc/on_entered(datum/source, atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	SIGNAL_HANDLER
	if(arrived == target)

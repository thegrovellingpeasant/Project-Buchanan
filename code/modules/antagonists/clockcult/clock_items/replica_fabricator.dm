...
/obj/item/clockwork/replica_fabricator/examine(mob/living/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		...
		if(uses_power)
			...
			. += span_alloy("It can also consume brass sheets for power at a rate of <b>1:[DisplayPower(POWER_FLOOR)]</b>.")
			. += span_alloy("Use it in-hand to produce <b>5</b> brass sheets at a cost of <b>[DisplayPower(POWER_WALL_TOTAL)]</b> power.")
			. += span_alloy("It has access to <b>[DisplayPower(get_clockwork_power())]</b> of power.")
...
/obj/item/clockwork/replica_fabricator/attack_self(mob/living/user)
	if(is_servant_of_ratvar(user))
		...
		to_chat(user, span_brass("You use [get_clockwork_power() ? "some":"all"] of [src]'s power to produce <b>5</b> brass sheets. It now has access to <b>[DisplayPower(get_clockwork_power())]</b> of power."))

...
/obj/effect/clockwork/judicial_marker/proc/judicialblast()
	...
	for(var/mob/living/L in range(1, src))
		...
		if(!iscultist(L))
			...
			L.visible_message(span_warning("[L] is struck by a judicial explosion!"), \
			span_heavy_brass("\"Keep an eye out, filth.\"</span>\n<span class='userdanger'>A burst of heat crushes you against the ground!"))
	...
	if(targetsjudged)
		to_chat(user, span_brass(span_bold("Successfully judged [span_neovgre("[targetsjudged]")] heretic[targetsjudged == 1 ? "":"s"].")))
	else
		to_chat(user, span_brass(span_bold("Judged no heretics.")))

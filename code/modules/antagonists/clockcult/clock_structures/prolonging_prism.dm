...
/obj/structure/destructible/clockwork/powered/prolonging_prism/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		if(SSshuttle.emergency.mode == SHUTTLE_DOCKED || SSshuttle.emergency.mode == SHUTTLE_IGNITING || SSshuttle.emergency.mode == SHUTTLE_STRANDED || SSshuttle.emergency.mode == SHUTTLE_ESCAPE)
			...
			. += span_inathneq_small("It requires at least <b>[DisplayPower(get_delay_cost())]</b> of power to attempt to delay the arrival of an emergency shuttle by <b>[2 * efficiency]</b> minutes.")
			. += span_inathneq_small("This cost increases by <b>[DisplayPower(delay_cost_increase)]</b> for every previous activation.")

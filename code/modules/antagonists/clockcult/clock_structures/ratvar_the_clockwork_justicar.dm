...
/obj/structure/destructible/clockwork/massive/ratvar/process()
	...
	if(!prey)
		if(!prey && LAZYLEN(meals))
			...
			prey = pick(meals)
			to_chat(prey, "[span_heavy_brass("<font size=5>\"You will do, heretic.\"</font>")]\n\
		...
		if((!istype(prey, /obj/singularity/narsie) && prob(10) && LAZYLEN(meals) > 1) || prey.z != z || !(prey in meals))
			if(is_servant_of_ratvar(prey))
				to_chat(prey, "[span_heavy_brass("<font size=5>\"Serve me well.\"</font>")]\n\
				[span_large_brass("You feel great joy as your god turns His eye to another heretic...")]")
			else
				to_chat(prey, "[span_heavy_brass("<font size=5>\"No matter. I will find you later, heretic.\"</font>")]\n\
...
/obj/structure/destructible/clockwork/massive/ratvar/proc/clash_of_the_titans(obj/singularity/narsie/narsie)
	...
	switch(winner)
		if("Ratvar")
			send_to_playing_players("[span_heavy_brass("<font size=5>\"[pick("DIE.", "ROT FOR CENTURIES, AS I HAVE!.","PERISH, HEATHEN.", "DIE, MONSTER, YOU DON'T BELONG IN THIS WORLD.")]\"</font>")]\n\
			[span_cult("<font size=5>\"<b>[pick("Nooooo...", "Not die. To y-", "Die. Ratv-", "Sas tyen re-")]\"</b></font>")]") //Nar'Sie get out
			...
			qdel(narsie)
		if("Nar'Sie")
			send_to_playing_players("[span_cult("<font size=5>\"<b>[pick("Ha.", "Ra'sha fonn dest.", "You fool. To come here.")]</b>\"</font>")]\n\
			[span_heavy_brass("<font size=5>\"[pick("NO, YOUR SHADOWS SHALL NO-", "ZNL GUR FGERNZF BS GVZR PNEEL ZL RKVFG-", "MY LIGHT CANNO-")]\"</font>")]")

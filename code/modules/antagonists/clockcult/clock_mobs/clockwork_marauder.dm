...
/mob/living/simple_animal/hostile/clockwork/marauder/guardian/proc/guardian_comms(message)
	...
	to_chat(src, "[name_part][span_sevtug_small(":")] [message]")
	to_chat(host, "[name_part][span_sevtug_small(":")] [message]")
	for(var/M in GLOB.mob_list)
		if(isobserver(M))
			var/link = FOLLOW_LINK(M, src)
			to_chat(M, "[link] [name_part] [span_sevtug_small("(to")] [span_sevtug("[findtextEx(host.name, host.real_name) ? "[host.name]" : "[host.real_name] (as [host.name])"]")][span_sevtug_small("):")] [message] ")
	return TRUE
...
/datum/action/innate/linked_minds/Activate()
	...
	message = span_sevtug_small("\"[message]\"") //Processed output
	to_chat(owner, "[name_part][span_sevtug_small(":")] [message]")
	to_chat(linked_guardian, "[name_part][span_sevtug_small(":")] [message]")
	for(var/M in GLOB.mob_list)
		if(isobserver(M))
			var/link = FOLLOW_LINK(M, src)
			to_chat(M, "[link] [name_part] [span_sevtug_small("(to")] [span_sevtug("[linked_guardian] ([linked_guardian.true_name])")][span_sevtug_small("):")] [message]")
	return TRUE

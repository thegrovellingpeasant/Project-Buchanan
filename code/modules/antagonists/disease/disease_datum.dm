...
/datum/antagonist/disease/roundend_report()
	...
	for(var/datum/objective/objective in objectives)
		if(objective.completable)
			...
			if(completion >= 1)
				result += "<B>Objective #[count]</B>: [objective.explanation_text] [span_greentext("<B>Success!</B>")]"
			else if(completion <= 0)
				result += "<B>Objective #[count]</B>: [objective.explanation_text] [span_redtext("Fail.")]"
				win = FALSE
			else
				result += "<B>Objective #[count]</B>: [objective.explanation_text] [span_yellowtext("[completion*100]%")]"
		else

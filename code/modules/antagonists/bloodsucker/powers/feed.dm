...
/datum/action/bloodsucker/feed/ActivatePower()
	...
	if(!do_mob(user, target, feed_time, 0, 1, extra_checks = CALLBACK(src, PROC_REF(ContinueActive), user, target)))
	...
	if(amSilent)
		...
		user.visible_message(
			message = span_notice("[user] puts [target]'s wrist up to [user.p_their()] mouth."), \
			self_message = span_notice("You secretly slip your fangs into [target]'s wrist.[deadmessage]"), \
			vision_distance = notice_range,
			ignored_mobs = target,
	...
	if(amSilent)
		...
		user.visible_message(
			message = span_warning("[user] unclenches their teeth from [target]'s neck."),
			self_message = span_warning("You retract your fangs and release [target] from your bite."),
		)

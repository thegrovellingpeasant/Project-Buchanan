...
//	owner.teach_crafting_recipe(/datum/crafting_recipe/bloodsucker/candelabrum)
	...
	to_chat(owner, "[span_announce("Bloodsucker Tip: Find new lair recipes in the misc tab of the <i>Crafting Menu</i> at the bottom of the screen, including the <i>Persuasion Rack</i> for converting crew into Vassals.")]<br><br>")
	RunLair() // Start
	return TRUE
...
/obj/structure/closet/crate/proc/UnclaimCoffin()
	if (resident)
		...
		if (resident.mind)
			...
			to_chat(resident, span_danger("[span_italics("You sense that the link with your coffin, your sacred place of rest, has been broken! You will need to seek another.")]"))

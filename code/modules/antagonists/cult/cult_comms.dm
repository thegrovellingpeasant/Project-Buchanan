...
/proc/pollCultists(mob/living/Nominee,datum/team/cult/team) //Cult Master Poll
	...
	for(var/datum/mind/B in team.members)
		if(B.current)
			...
			if(!B.current.incapacitated())
				to_chat(B.current,span_cult_large("[Nominee] has won the cult's support and is now their master. Follow [Nominee.p_their()] orders to the best of your ability!"))
	return TRUE
...
/datum/action/innate/cult/master/cultmark/IsAvailable(silent = FALSE)
	if(cooldown > world.time)
		if(!CM.active && !silent)
			to_chat(owner, span_cult_large("<b>You need to wait [DisplayTimeText(cooldown - world.time)] before you can mark another target!</b>"))
		return FALSE
	return ..()
...
/obj/effect/proc_holder/cultmark/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	if(!C.cult_team)
		to_chat(ranged_ability_user, span_cult_large("What is the point of marking a target for yourself?"))
		remove_ranged_ability()
		return
	if(target in view(7, get_turf(ranged_ability_user)))
		...
		for(var/datum/mind/B in SSticker.mode.cult)
			if(B.current && B.current.stat != DEAD && B.current.client)
				to_chat(B.current, span_cult_large("<b>[ranged_ability_user] has marked [C.cult_team.blood_target] in the [A.name] as the cult's top priority, get there immediately!</b>"))
	...
	for(var/datum/mind/B in team.members)
		if(B.current && B.current.stat != DEAD && B.current.client)
			if(team.blood_target)
				to_chat(B.current,span_cult_large("<b>The blood mark has expired!</b>"))
...
/datum/action/innate/cult/ghostmark/Activate()
	var/datum/antagonist/cult/C = owner.mind.has_antag_datum(/datum/antagonist/cult,TRUE)
	if(!C.cult_team)
		to_chat(owner, span_cult_bold("You are alone. You do not have a team."))
		return
	if(C.cult_team.blood_target)
		if(cooldown > world.time)
	...
	for(var/datum/mind/B in SSticker.mode.cult)
		if(B.current && B.current.stat != DEAD && B.current.client)
			to_chat(B.current, span_cult_large("<b>[owner] has marked [C.cult_team.blood_target] in the [A.name] as the cult's top priority, get there immediately!</b>"))
			SEND_SOUND(B.current, sound(pick('sound/hallucinations/over_here2.ogg','sound/hallucinations/over_here3.ogg'),0,1,75))
			B.current.client.images += C.cult_team.blood_target_image
	to_chat(owner,span_cult_bold("You have marked the [target] for the cult! It will last for [DisplayTimeText(base_cooldown)]."))
...
/datum/action/innate/cult/master/pulse/IsAvailable(silent = FALSE)
	...
	if(cooldown > world.time)
		if(!PM.active && !silent)
			to_chat(owner, span_cult_large("<b>You need to wait [DisplayTimeText(cooldown - world.time)] before you can pulse again!</b>"))
		return FALSE
	return ..()

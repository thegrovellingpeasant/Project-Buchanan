...
/datum/action/innate/cult/blood_magic/Activate()
	if(holy_dispel)
		to_chat(owner, span_cult_bold("Holy water currently scours your body, nullifying the power of the rites!"))
		return
	var/rune = FALSE
	...
	if(spells.len >= limit)
		if(rune)
			to_chat(owner, span_cult_italic("Your body has reached its limit, you cannot store more than [MAX_BLOODCHARGE] spells at once. <b>Pick a spell to nullify.</b>"))
		else
			to_chat(owner, span_cult_italic("Your body has reached its limit, <b><u>you cannot have more than [RUNELESS_MAX_BLOODCHARGE] spells at once without an empowering rune! Pick a spell to nullify.</b></u>"))
...
/datum/action/innate/cult/blood_spell/equipment/Activate()
	...
	if(choice == "Ritual Dagger")
		...
		owner.visible_message(span_warning("[owner]'s hand glows red for a moment."), \
			span_cult_italic("Red light begins to shimmer and take form within your hand!"))
		...
		if(owner.put_in_hands(O))
			...
			owner.visible_message(span_warning("A ritual dagger appears at [owner]'s feet!"), \
				span_cult_italic("A ritual dagger materializes at your feet."))
...
/obj/item/melee/blood_magic/stun/afterattack(atom/target, mob/living/carbon/user, proximity)
	...
	if(iscultist(user))
		user.visible_message(span_warning("[user] holds up [user.p_their()] hand, which explodes in a flash of red light!"), \
							span_cult_italic("You attempt to stun [L] with the spell!"))
		...
		if(anti_magic_source)
			...
			to_chat(user, span_cult_italic("In an brilliant flash of red, [L] [iscultist(L) ? "writhes in pain" : "falls to the ground!"]"))
		uses--
	..()
...
/obj/item/melee/blood_magic/manipulator/examine(mob/user)
	. = ..()
	if(iscultist(user))
		. += span_cult_italic("The [name] currently has <b>[uses]</b> blood charges left.")
...
/obj/item/melee/blood_magic/manipulator/proc/blood_draw(atom/target, mob/living/carbon/human/user)
	...
	if(T)
		...
		if(temp)
			...
			to_chat(user, span_cult_italic("Your blood rite has gained [round(temp)] charge\s from blood sources around you!"))
...
/obj/item/melee/blood_magic/manipulator/attack_self(mob/living/user)
	if(iscultist(user))
		...
		switch(choice)
			if("Blood Spear (150)")
				if(uses < 150)
					to_chat(user, span_cult_italic("You need 150 charges to perform this rite."))
				else
					uses -= 150
			...
			if("Blood Bolt Barrage (300)")
				if(uses < 300)
					to_chat(user, span_cult_italic("You need 300 charges to perform this rite."))
				else
			...
			if("Blood Beam (500)")
				if(uses < 500)
					to_chat(user, span_cult_italic("You need 500 charges to perform this rite."))

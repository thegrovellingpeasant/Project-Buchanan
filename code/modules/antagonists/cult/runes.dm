...
/obj/effect/rune/convert/invoke(list/invokers)
	...
	if(L.stat != DEAD && (is_clock || is_convertable))
		...
		if(is_clock)
			...
			"[span_cult_large("\"Stop resisting. You <i>will</i> be mi-\"")]\n\
...
/obj/effect/rune/convert/proc/do_sacrifice(mob/living/sacrificial, list/invokers)
	...
	for(var/M in invokers)
		if(big_sac)
			to_chat(M, span_cult_large("\"Yes! This is the one I desire! You have done well.\""))
		else
			if(ishuman(sacrificial) || iscyborg(sacrificial))
				to_chat(M, span_cult_large("\"I accept this sacrifice.\""))
			else
				to_chat(M, span_cult_large("\"I accept this meager sacrifice.\""))
...
/obj/effect/rune/narsie/invoke(list/invokers)
	...
	if(!user_antag.cult_team)
		to_chat(user, span_cult_large("You can't seem to make the arcane links to your fellows that you'd need to use this."))
		return
...
/obj/effect/rune/wall/examine(mob/user)
	...
	if(density && iscultist(user))
		...
		if(TMR)
			. += span_cult_italic("The air above this rune has hardened into a barrier that will last [DisplayTimeText(TMR.timeToRun - world.time)].")
	...
	for(var/mob/living/L in viewers(T))
		if(!iscultist(L) && L.blood_volume)
			...
			if(I)
				...
				continue
			to_chat(L, span_cult_large("Your blood boils in your veins!"))

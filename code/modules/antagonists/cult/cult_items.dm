...
/obj/item/melee/cultblade/pickup(mob/living/user)
	...
	if(!iscultist(user))
		if(!is_servant_of_ratvar(user))
			...
			to_chat(user, span_cult_large("\"One of Ratvar's toys is trying to play with things [user.p_they()] shouldn't. Cute.\""))
...
/obj/item/clothing/suit/hooded/cultrobes/cult_shield/equipped(mob/living/user, slot)
	...
	if(!iscultist(user))
		if(!is_servant_of_ratvar(user))
			...
			to_chat(user, span_cult_large("\"Trying to use things you don't own is bad, you know.\""))
		...
		if(iscultist(L))
			...
			if(!L.anti_magic_check())
				if(is_servant_of_ratvar(L))
					to_chat(L, span_cult_large("\"Kneel for me, scum\""))

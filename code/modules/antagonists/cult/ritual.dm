...
/obj/item/melee/cultblade/dagger/proc/scribe_rune(mob/living/user)
	...
	if(ispath(rune_to_scribe, /obj/effect/rune/summon) && (!is_station_level(Turf.z) || A.map_name == "Space"))
		to_chat(user, span_cult_italic("<b>The veil is not weak enough here to summon a cultist, you must be on station!</b>"))
		return
	...
	if(ispath(rune_to_scribe, /obj/effect/rune/narsie))
		...
		if(summon_objective.check_completion())
			to_chat(user, span_cult_large("\"I am already here. There is no need to try to summon me now.\""))
			return
		if(!(A in summon_objective.summon_spots))

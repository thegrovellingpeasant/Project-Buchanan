...
/obj/structure/destructible/cult/talisman/ui_interact(mob/user)
	...
	if(!iscultist(user))
		...
		return
	if(!anchored)
		to_chat(user, span_cult_italic("You need to anchor [src] to the floor with your dagger first."))
		return
	if(cooldowntime > world.time)
		to_chat(user, span_cult_italic("The magic in [src] is weak, it will be ready to use again in [DisplayTimeText(cooldowntime - world.time)]."))
		return

	to_chat(user, span_cult_italic("You study the schematics etched into the altar..."))
	...
	if(!QDELETED(src) && reward && check_menu(user))
		...
		to_chat(user, span_cult_italic("You kneel before the altar and your faith is rewarded with the [choice]!"))
...
/obj/structure/destructible/cult/forge/ui_interact(mob/user)
	...
	to_chat(user, span_cult_italic("You study the schematics etched into the forge..."))
	...
	if(!QDELETED(src) && reward && check_menu(user))
		cooldowntime = world.time + 2400
		new reward(get_turf(src))
		to_chat(user, span_cult_italic("You work the forge as dark knowledge guides your hands, creating the [choice]!"))
...
/obj/structure/destructible/cult/tome/ui_interact(mob/user)
	...
	to_chat(user, span_cult_italic("You flip through the black pages of the archives..."))
	...
	if(!QDELETED(src) && reward && check_menu(user))
		cooldowntime = world.time + 2400
		new reward(get_turf(src))
		to_chat(user, span_cult_italic("You summon the [choice] from the archives!"))

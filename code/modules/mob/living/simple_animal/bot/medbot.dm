...
/mob/living/simple_animal/bot/medbot/assaultron
	name = "Assaultron"
	desc = "A medical robot."
	icon = 'icons/mob/aibots.dmi'
	icon_state = "assaultron0"

/mob/living/simple_animal/bot/medbot/assaultron/update_icon()
	cut_overlays()
	if(skin)
		add_overlay("medskin_[skin]")
	if(!on)
		...
		return
	if(IsStun())
		icon_state = "assaultrona"
		return
	if(mode == BOT_HEALING)
		icon_state = "assaultrons[stationary_mode]"
		return
	else if(stationary_mode) //Bot has yellow light to indicate stationary mode.
		icon_state = "assaultron2"
	else
		icon_state = "assaultron1"

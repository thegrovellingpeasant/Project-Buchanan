...
/mob/living/simple_animal/chicken/caged
	layer = OBJ_LAYER
	wander = FALSE

/mob/living/simple_animal/chicken/caged/Initialize()
	. = ..()
	if(!body_color)
		body_color = pick(validColors)
	icon_state = "[icon_prefix]_[body_color]"
...
/mob/living/simple_animal/cow/brahmin/CtrlShiftClick(mob/user)
	...
	if(user == owner)
		if(bridle && user.a_intent == INTENT_HELP)
			if(follow)
				to_chat(user, span_notice("You tug on the reins of [src], telling it to stop."))
				follow = FALSE
				return
			else if(!follow)

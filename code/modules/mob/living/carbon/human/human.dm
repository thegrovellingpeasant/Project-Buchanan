...
/mob/living/carbon/human/show_inv(mob/user)
	...
	for(var/i in 1 to held_items.len)
		...
		dat += "<tr><td><B>[get_held_index_name(i)]:</B></td><td><A href='?src=[REF(src)];item=[ITEM_SLOT_POCKETS];hand_index=[i]'>[(I && !(I.item_flags & ABSTRACT)) ? I : "<font color=grey>Empty</font>"]</a></td></tr>"
	dat += "<tr><td>&nbsp;</td></tr>"
	...
	dat += "<tr><td><B>Suit/Armor:</B></td><td><A href='?src=[REF(src)];item=[ITEM_SLOT_OCLOTHING]'>[(wear_suit && !(wear_suit.item_flags & ABSTRACT)) ? wear_suit : "<font color=grey>Empty</font>"]</A>"
	if(wear_suit)
		if(istype(wear_suit, /obj/item/clothing/suit/space/hardsuit))
			...
			dat += "&nbsp;<A href='?src=[REF(src)];toggle_helmet=[ITEM_SLOT_OCLOTHING]'>[hardsuit_head ? "Retract Helmet" : "Extend Helmet"]</A>"
		dat += "</td></tr>"
...
/mob/living/carbon/human/proc/on_entered(datum/source, atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	SIGNAL_HANDLER
	var/mob/living/simple_animal/bot/mulebot/MB = arrived
	if(istype(MB))

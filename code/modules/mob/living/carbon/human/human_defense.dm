...
/mob/living/carbon/human/attack_alien(mob/living/carbon/alien/humanoid/M)
	...
	if(M.a_intent == INTENT_HARM)
		...
		if(!damage)
			...
			visible_message(span_danger("[M] has lunged at [src]!"), \
				span_userdanger("[M] has lunged at you!"))
			return 0
	...
	if(M.a_intent == INTENT_DISARM) //Always drop item in hand, if no item, get stun instead.
		...
		if(I && dropItemToGround(I))
			playsound(loc, 'sound/weapons/slash.ogg', 25, 1, -1)
			visible_message(span_danger("[M] has disarmed [src]!"), \
					span_userdanger("[M] has disarmed you!"))
		else
			playsound(loc, 'sound/weapons/pierce.ogg', 25, 1, -1)
			DefaultCombatKnockdown(M.meleeKnockdownPower)
			log_combat(M, src, "tackled")
			visible_message(span_danger("[M] has tackled down [src]!"), \
				span_userdanger("[M] has tackled you down!"))
...
/mob/living/carbon/human/mech_melee_attack(obj/mecha/M)
	if(M.occupant.a_intent == INTENT_HARM)
		...
		visible_message(span_danger("[M.name] has hit [src]!"), \
						span_userdanger("[M.name] has hit you!"))
		...
		if(src == M)
				...
				for(var/thing in LB.wounds)
					...
					switch(W.severity)
						if(WOUND_SEVERITY_TRIVIAL)
							msg = "\t [span_danger("Your [LB.name] is suffering [W.a_or_from] [lowertext(W.name)].")]"
						if(WOUND_SEVERITY_MODERATE)
							msg = "\t [span_warning("Your [LB.name] is suffering [W.a_or_from] [lowertext(W.name)]!")]"
						if(WOUND_SEVERITY_SEVERE)
							msg = "\t [span_warning("<b>Your [LB.name] is suffering [W.a_or_from] [lowertext(W.name)]!</b>")]"
			...
			switch(nutrition)
				...
				if(NUTRITION_LEVEL_DEADLY_STARVING to NUTRITION_LEVEL_STARVING)
					to_send += "[span_warning("You're starving!")]\n"
				if(0 to NUTRITION_LEVEL_DEADLY_STARVING)
					to_send += "[span_danger("You're starving to death!")]\n"

			switch(thirst)
				if((THIRST_LEVEL_FULL - 50) to INFINITY)
					to_send += "[span_info("You're not thirsty at all!")]\n"
				if(THIRST_LEVEL_NORMAL to (THIRST_LEVEL_FULL - 50))
					to_send += "[span_info("You could use a drink.")]\n"
				if(THIRST_LEVEL_THIRSTY to THIRST_LEVEL_NORMAL)
					to_send += "[span_info("You are slightly thirsty.")]\n"
				if(THIRST_LEVEL_DEADLY_THIRSTY to THIRST_LEVEL_THIRSTY)
					to_send += "[span_warning("You need water!")]\n"
				if(0 to THIRST_LEVEL_DEADLY_THIRSTY)
					to_send += "[span_danger("You really need some water!")]\n"

...
/datum/component/two_handed/proc/on_equip(datum/source, mob/user, slot)
	...
	if(require_twohands && (slot == ITEM_SLOT_POCKET)S) // force equip the item
		wield(user)
...
/datum/component/two_handed/proc/wield(mob/living/carbon/user)
	...
	if(ismonkey(user))
		if(require_twohands)
			to_chat(user, span_notice("You can't wield [parent]!"))
			user.dropItemToGround(parent, force = TRUE)
		else
			to_chat(user, span_notice("You can't carry [parent] in both hands!"))
		return
	...
	if(user.get_num_arms() < 2)
		if(require_twohands)
			user.dropItemToGround(parent, force = TRUE)
		to_chat(user, span_notice("You don't have enough hands to weild [parent]!"))
		return

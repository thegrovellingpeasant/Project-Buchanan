...
/datum/wound/burn/get_examine_description(mob/user)
	if(strikes_to_lose_limb <= 0)
		return span_deadsay("<B>[victim.p_their(TRUE)] [limb.name] is completely dead and unrecognizable as organic.</B>")
	...
	if(limb.current_gauze)
		...
		switch(infestation)
			if(WOUND_INFECTION_MODERATE to WOUND_INFECTION_SEVERE)
				condition += ", [span_deadsay("with small spots of discoloration along the nearby veins!")]"
			if(WOUND_INFECTION_SEVERE to WOUND_INFECTION_CRITICAL)
				condition += ", [span_deadsay("with dark clouds spreading outwards under the skin!")]"
			if(WOUND_INFECTION_CRITICAL to WOUND_INFECTION_SEPTIC)
				condition += ", [span_deadsay("with streaks of rotten infection pulsating outward!")]"
			if(WOUND_INFECTION_SEPTIC to INFINITY)
				return span_deadsay("<B>[victim.p_their(TRUE)] [limb.name] is a mess of char and rot, skin literally dripping off the bone with infection!</B>")
			else
				condition += "!"
...
/datum/wound/burn/get_scanner_description(mob/user)
	if(strikes_to_lose_limb == 0)
		...
		oopsie += "<div class='ml-3'>Infection Level: [span_deadsay("The infection is total. The bodypart is lost. Amputate or augment limb immediately.")]</div>"
		return oopsie

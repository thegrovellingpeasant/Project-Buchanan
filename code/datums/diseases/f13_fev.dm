...
/datum/disease/transformation/mutant/do_disease_transformation(mob/living/affected_mob)
	...
	if(!ispath(new_form, /mob/living/carbon)) // If you've become simple_mob - you can't go and be all friendly to those around you!
		to_chat(affected_mob, "<big>[span_warning("<b>You've become something entirely different! You are being controlled only by your hunger and desire to kill!</b>")]</big>")
	. = ..()
	...
	switch(stage)
		...
		if(5) // That's the part where you die for real
			...
			if(prob(5))
				...
				affected_mob.emote("scream")
			if(prob(6))
				to_chat(affected_mob, span_userdanger("You want to rest. Forever..."))
			...
			if(prob(7))
				...
				affected_mob.emote("twitch")
			if(prob(10))
				to_chat(affected_mob, span_userdanger("You feel a sharp pain from your lower chest!"))

...
/obj/mecha/emp_act(severity)
	...
	addtimer(CALLBACK(src, PROC_REF(restore_equipment)), 3 SECONDS, TIMER_UNIQUE | TIMER_OVERRIDE)
...
/obj/mecha/attackby(obj/item/W as obj, mob/user as mob, params)
	...
	if(W.GetID())
		...
		if(state==4)
			...
			to_chat(user, span_notice("You replace the scanning module."))
		...
		if(state==4)
			...
			to_chat(user, span_notice("You replace the capacitor."))

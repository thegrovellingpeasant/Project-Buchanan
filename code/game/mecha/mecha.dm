...
/obj/mecha/process()
	...
	if(occupant)
		...
		switch(integrity)
			if(30 to 45)
				if(wastebot)
					occupant.throw_alert("mech damage", /atom/movable/screen/alert/low_mech_integrity/robot, 1)
				else
					occupant.throw_alert("mech damage", /atom/movable/screen/alert/low_mech_integrity, 1)
			if(15 to 35)
				if(wastebot)
					occupant.throw_alert("mech damage", /atom/movable/screen/alert/low_mech_integrity/robot, 2)
				else
					occupant.throw_alert("mech damage", /atom/movable/screen/alert/low_mech_integrity, 2)
			if(-INFINITY to 15)
				if(wastebot)
					occupant.throw_alert("mech damage", /atom/movable/screen/alert/low_mech_integrity/robot, 3)
				else
...
/obj/mecha/proc/mmi_move_inside(obj/item/mmi/mmi_as_oc, mob/user)
	...
	if(!mmi_as_oc.brainmob || !mmi_as_oc.brainmob.client)
		to_chat(user, span_warning("Personality matrix not detected!"))
		return FALSE
	...
	visible_message(span_notice("[user] starts to insert an personality matrix into [name]."))
	...
	if(do_after(user, exit_delay, target = src))
		...
		to_chat(user, span_notice("You stop inserting the personality matrix."))
	return FALSE

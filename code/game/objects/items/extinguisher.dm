...
/obj/item/extinguisher/afterattack(atom/target, mob/user , flag)
	...
	if (!safety)
		...
		if(user.buckled && isobj(user.buckled) && !user.buckled.anchored)
			...
			addtimer(CALLBACK(src, PROC_REF(move_chair), B, movementdirection), 1)
		...
		addtimer(CALLBACK(src, PROC_REF(move_particles), water_particles), 2)
...
/obj/item/extinguisher/proc/move_particles(list/particles, repetition=0)
	...
	if(repetition < power)
		repetition++
		addtimer(CALLBACK(src, PROC_REF(move_particles), particles, repetition), 2)
...
/obj/item/extinguisher/proc/move_chair(obj/B, movementdirection, repetition=0)
	...
	repetition++
	addtimer(CALLBACK(src, PROC_REF(move_chair), B, movementdirection, repetition), timer_seconds)

...
/obj/item/card/id/prisoner/GetAccess()
	if((sentence && world.time >= sentence) || (goal && points >= goal))
		to_chat(FREQ_NCRCF, span_boldnotice("A notification</span><span class='notice'> buzzes: A prisoner has served their sentence, please facilitate the departure of: [registered_name]."))
	return ..()
...
/obj/item/card/id/prisoner/process()
	...
	if(world.time >= sentence)
		...
		if(isliving(loc))
			to_chat(loc, span_boldnotice("[src]</span><span class='notice'> buzzes: You have served your sentence! You may now exit prison through the turnstiles and collect your belongings."))
		STOP_PROCESSING(SSobj, src)
	return

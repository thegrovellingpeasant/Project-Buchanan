...
/obj/item/stack/sheet/leather/attackby(obj/item/W, mob/user, params)
	if(W.get_sharpness())
		playsound(loc, 'sound/weapons/slice.ogg', 50, 1, -1)
		user.visible_message("[user] starts shredding the [src] into strips.", span_notice("You start shredding the [src] into strips"), span_italics("You hear the sound of leather being ripped."))
		if(do_after(user, 50, target = src))

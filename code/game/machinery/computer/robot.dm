	...
	switch(action)
		if("killbot")
			...
			if(can_control(usr, R) && !..())
				...
				message_admins(
					span_notice("[ADMIN_LOOKUPFLW(usr)] detonated [key_name_admin(R, R.client)] at [ADMIN_VERBOSEJMP(T)]!"),
				)
		...
		if("stopbot")
			...
			if(can_control(usr, R) && !..())
				...
				if(R.connected_ai)
					to_chat(R.connected_ai, "[!R.locked_down ? span_notice("NOTICE - Cyborg lockdown lifted") : span_alert("ALERT - Cyborg lockdown detected")]: <a href='?src=[REF(R.connected_ai)];track=[html_encode(R.name)]'>[R.name]</a>")
		if("magbot")

...
/obj/machinery/computer/card/ui_interact(mob/user)
	...
	if (authenticated && inserted_modify_id)
		...
		else if(istype(src, /obj/machinery/computer/card/eighties))
			accesses += "<h5>80s:</h5>"
			for(var/A in get_all_eighties_access())
				if(A in inserted_modify_id.access)
					accesses += "<a href='?src=[REF(src)];choice=access;access_target=[A];allowed=0'><font color=\"6bc473\">[replacetext(get_eighties_access_desc(A), " ", "&nbsp")]</font></a> "
				else
					accesses += "<a href='?src=[REF(src)];choice=access;access_target=[A];allowed=1'>[replacetext(get_eighties_access_desc(A), " ", "&nbsp")]</a> "
		else

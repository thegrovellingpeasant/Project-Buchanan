...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal
	...
	name = "correctional records terminal"
	desc = "An NCRCF terminal, it functions as a security records console. It's bolted to the table."
	icon_state = "terminal"
	icon_keyboard = "terminal_key"
	icon_screen = "terminal_on_alt"
	req_one_access = list(ACCESS_NCR)
	var/id			// id of linked prisoners
...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/Initialize()
	. = ..()

	Radio = new/obj/item/radio(src)
	Radio.listening = 0

/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/ui_interact(mob/user)
	. = ..()
	if(isliving(user))
		playsound(src, 'sound/machines/terminal_prompt_confirm.ogg', 50, FALSE)
	if(src.z > 6)
	...
	if(temp)
		...
		<head>
			...
			<script type='text/javascript'>
				...
				// Sentence Time
				const cd = new Date()
				release_time = world.timeofday + [round(active1.fields["sentence"])] * 10
				const releaseTime = new Date(cd.getFullYear(), cd.getMonth(), cd.getDate(), [round(release_time / (10*60*60))], [round(release_time % (10*60*60) / (10*60))],[round(release_time % (10*60) / 10)],[round(release_time % 10 * 100)]).getTime();

				// Interval Updates "function()" by Second
				var x = setInterval(function() {
					// Get today's date and time
					const currentTime = new Date().getTime();

					// Time till Release
					var delta = releaseTime - currentTime;
					
					// Calculate seconds, minutes, and hours from "delta"
					...
					// Display time
					if(delta > 0) {
						document.getElementById("sentence").innerHTML = hours + ":" + minutes + ":" + seconds + " (h:m:s)";
					} else {
						document.getElementById("sentence").innerHTML = "RELEASE";
					}
				}, 1000);
			</script>
		</head>
...
<th>Criminal Status</th>
				...
				if(3)
					...
					if(istype(active1, /datum/data/record) && GLOB.data_core.general.Find(active1))
						...
						<tr><td>Physical Status:</td><td>&nbsp;[active1.fields["p_stat"]]&nbsp;</td></tr>
						<tr><td>Mental Status:</td><td>&nbsp;[active1.fields["m_stat"]]&nbsp;</td></tr>
						<tr><td>Sentence:</td><td><A href='?src=[REF(src)];choice=Edit Field;field=sentence'>&nbsp;[active1.fields["sentence"]]&nbsp;</A></td></tr>
						</table></td>
						<td><table><td align = center><a href='?src=[REF(src)];choice=Edit Field;field=show_photo_front'><img src=photo_front height=80 width=80 border=4></a><br>
						<a href='?src=[REF(src)];choice=Edit Field;field=print_photo_front'>Print photo</a><br>
						<a href='?src=[REF(src)];choice=Edit Field;field=upd_photo_front'>Update front photo</a></td>
...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/Topic(href, href_list)
	. = ..()
	if(.)
		return .
	if(!( GLOB.data_core.general.Find(active1) ))
			...
			if("Print Record")
				if(!( printing ))
					...
					if((istype(active1, /datum/data/record) && GLOB.data_core.general.Find(active1)))
						P.info += text("Name: [] ID: []<BR>\nGender: []<BR>\nAge: []<BR>", active1.fields["name"], active1.fields["id"], active1.fields["gender"], active1.fields["age"])
						P.info += "\nSpecies: [active1.fields["species"]]<BR>"
						P.info += text("\nFingerprint: []<BR>\nPhysical Status: []<BR>\nMental Status: []<BR>\nSentence: []<BR>", active1.fields["fingerprint"], active1.fields["p_stat"], active1.fields["m_stat"], active1.fields["sentence"])
					else
						P.info += "<B>General Record Lost!</B><BR>"
					if((istype(active2, /datum/data/record) && GLOB.data_core.security.Find(active2)))
						P.info += text("<BR>\n<CENTER><B>Security Data</B></CENTER><BR>\nCriminal Status: []", active2.fields["criminal"])
...
//RECORD CREATE
			if("New Record (Security)")
				if((istype(active1, /datum/data/record) && !( istype(active2, /datum/data/record) )))
					...
					R.fields["ma_crim"] = list()
					R.fields["notes"] = "No notes."
					R.fields["sentence"] = active1.fields["sentence"]
					GLOB.data_core.security += R
					active2 = R
					screen = 3
...
//FIELD FUNCTIONS
			if("Edit Field")
				...
				switch(href_list["field"])
					...
					if("sentence")
						if(istype(active1, /datum/data/record))
							var/t1 = input("Input their sentence:", "Secure. records", active1.fields["sentence"], null) as num
							if(!canUseSecurityRecordsConsole(usr, "sentence", a1))
								return
							//these records suck. why do I work on this
							// please god let it end. I hate the timer system.
							// I set the variables in the wrong place. I need to do this shit over again. the fog is coming.
							// this sucks but I need to make itw ork.
							active1.fields["sentence"] = t1
...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/process()
...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/proc/set_timer(value)
	var/new_time = clamp(value,0,MAX_TIMER)
	. = new_time == timer_duration //return 1 on no change
	timer_duration = new_time

/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/proc/timer_start()
...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/proc/timer_end(forced = FALSE)
	...
	if(!forced)
		...
		Radio.talk_into(src, "Timer has expired. Please escort the prisoner.", FREQ_SECURITY)
...
/obj/machinery/computer/secure_data/laptop/ncrcf_terminal/proc/time_left(seconds = FALSE)
